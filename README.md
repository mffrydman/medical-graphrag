...
- [Medical GraphRAG](#medical-graphrag)
...
A comprehensive GraphRAG (Graph Retrieval-Augmented Generation) system designed for medical research. It combines knowledge graphs with vector search to provide intelligent querying and analysis of medical literature and genomic data.

**Key Features:**

- **Hybrid Query System**: Combines Neo4j graph database with Qdrant vector search for comprehensive medical insights
...
- **Medical Schema**: Specialized graph schema for papers, authors, institutions, genes, and MeSH terms
...
1. Clone the repository:
   ...
   ```bash
   git clone git@github.com:mffrydman/medical-graphrag.git
   cd medical-graphrag
   ```
...
The system includes data collectors for medical and gene datasets:
...
make custom-qdrant-query QUESTION="Which institutions have collaborated most frequently on papers about 'Dementia' and 'Alzheimer's Disease'?"

# Or run directly with the CLI
uv run src/medical_graphrag/application/cli/query_vectorstore.py --ask "Which institutions have collaborated most frequently on papers about 'Dementia' and 'Alzheimer's Disease'?"
```
